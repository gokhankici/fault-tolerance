...
- They *assumed* that Raft's state machine safety property *holds*, and proved
  that it implies *linearizability* of the Raft. Their proof of Raft's state
  machine safety was still in progress.

- *Generalized theory of linearizable systems*: A trace \tau of requests I_1,...,I_n and responses O_1,...,O_m (where there is
...
- Stronger idea of leadership :)
- Built in leader election using randomized timers
- Handles membership changes
  
*Paxos drawbacks:*
- Difficult to understand
- No good foundation for building practical implementations. 
- There's no widely agreed-upon algorithm for multi-Paxos.
- Uses a symmetric peer-to-peer approach, only suggests weak form of leadership
  for performance optimization
- Implementations generally begin with Paxos but later develops into a different
  architecture due to difficulties
  
*Goals of Raft:*
1. Understandable
2. Complete and practical
3. Efficient for common operations
   
*** Algorithm

- *Leader election*: A new leader must be chosen when an existing leader fails
- *Log replication*: The leader must accept log entries from clients and replicate
  them across the cluster, forcing others to agree with its own
- *Safety properties*:
  - *At most one leader* can be elected in a term. During a *split vote*, the
    term will end with no leader; a new term with a new election will begin.
  - A leader *only appends* new entries in its log
  - If two logs contain an entry with the same index and term, then the logs are
    *identical in all terms up through* the given index
  - If a log entry is committed in a given term, it'll be *present in the higher
    numbered terms* as well
...
_Followers:_
- Respond to RPC's from candidates or leaders
...
_Leaders:_
- Upon election: send initial empty =AppendEntries= RPC's (*heartbeat*) to each
...
- If last log index \geq =nextIndex= for a follower: send =AppendEntries= RPC
  ...
  - If =AppendEntries= fails because of *log inconsistency*: decrement =nextIndex=
    and retry
- If \exists N, N > =commitIndex=, a majority of =matchIndex[i]= \geq N and
  =log[N].term =  currentTerm=, set =commitIndex = N=

*Notes:*
- Each server *vote for at most one candidate* in a given term

- To *prevent split votes* in the first place, election timeouts are chosen
  *randomly* from a fixed interval (e.g., 150–300ms). Each candidate restarts
  its randomized election timeout at the start of an election, and it waits for
...
- Raft uses the voting process to prevent a candidate from winning an election
  *unless its log contains all committed entries*. Voters denies its vote if its
  own log is more up-to-date than that of the candidate

- Raft will be able to elect and maintain a steady leader as long as the system
  satisfies the following *timing requirements*:
  
  =broadcastTime= << =electionTime= << MTBF

  ex: broadcast=0.5-20ms, election=10-500ms, MTBF=several months
...
- interactive
- infinite state
- user generalizes the proof using the counter-example
- searches for universally qualified invariants
- uses restrictive modeling language, this ensures that all verification
  conditions can be checked algorithmically
- has a GUI

*** Introduction
    
- They claim that current automated methods are *opaque*, they should fail
  *visibly*
- 
